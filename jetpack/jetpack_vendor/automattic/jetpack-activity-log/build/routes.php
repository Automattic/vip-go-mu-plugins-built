...
foreach ( $routes_by_page as $page_slug => $page_routes ) {
	...
	$global_name = 'jetpack_activity_log_' . $page_slug_underscore . '_routes_data';
 ...
 */
function jetpack_activity_log_register_page_routes( $page_routes, $register_function_name ) {
	...
	foreach ( $page_routes as $route ) {
		...
		if ( $route['has_content'] ) {
			...
			if ( file_exists( $content_asset_path ) ) {
				...
				$content_handle = 'jetpack-activity-log/routes/' . $route['name'] . '/content';
				$extension = '.min.js';
		...
		if ( $route['has_route'] ) {
			...
			if ( file_exists( $route_asset_path ) ) {
				...
				$route_handle = 'jetpack-activity-log/routes/' . $route['name'] . '/route';
				$extension = '.min.js';
				// Deregister first to override any previously registered version
				// (e.g., Core's default modules when running as a plugin).
...
// Page-specific route registration functions for jetpack-activity-log-dashboard
/**
 * Register routes for jetpack-activity-log-dashboard page (full-page mode).
 */
function jetpack_activity_log_register_jetpack_activity_log_dashboard_page_routes() {
	global $jetpack_activity_log_jetpack_activity_log_dashboard_routes_data;
	jetpack_activity_log_register_page_routes( $jetpack_activity_log_jetpack_activity_log_dashboard_routes_data, 'jetpack_activity_log_register_jetpack_activity_log_dashboard_route' );
}
add_action( 'jetpack-activity-log-dashboard_init', 'jetpack_activity_log_register_jetpack_activity_log_dashboard_page_routes' );

/**
 * Register routes for jetpack-activity-log-dashboard page (wp-admin mode).
 */
function jetpack_activity_log_register_jetpack_activity_log_dashboard_wp_admin_page_routes() {
	global $jetpack_activity_log_jetpack_activity_log_dashboard_routes_data;
	jetpack_activity_log_register_page_routes( $jetpack_activity_log_jetpack_activity_log_dashboard_routes_data, 'jetpack_activity_log_register_jetpack_activity_log_dashboard_wp_admin_route' );
}
add_action( 'jetpack-activity-log-dashboard-wp-admin_init', 'jetpack_activity_log_register_jetpack_activity_log_dashboard_wp_admin_page_routes' );
