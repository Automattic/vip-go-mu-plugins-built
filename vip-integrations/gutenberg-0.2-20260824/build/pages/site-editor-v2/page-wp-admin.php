...
function gutenberg_site_editor_v2_wp_admin_enqueue_scripts( $hook_suffix ) {
	...
	/**
	 * Fires when the site-editor-v2 admin page is initialized so extensions can register routes and menu items.
	 */
	...
	if ( file_exists( $asset_file ) ) {
		...
		wp_add_inline_script(
			...
			sprintf(
				'( %s )( %s, %s, %s );',
				$init_js_function,
				wp_json_encode( 'site-editor-v2-wp-admin-app', JSON_HEX_TAG | JSON_UNESCAPED_SLASHES ),
		...
		$boot_dependencies = apply_filters(
			'site-editor-v2-wp-admin_boot_dependencies',
			$boot_dependencies
		);
...
function gutenberg_site_editor_v2_wp_admin_render_page() {
	...
	<style>
		...
		body.js #wpbody-content > div:not(#site-editor-v2-wp-admin-app):not(#screen-meta) {
			display: none;
		}
		body.js #wpfooter {
			display: none;
	...
	<div id="site-editor-v2-wp-admin-app"></div>
	<?php
}
