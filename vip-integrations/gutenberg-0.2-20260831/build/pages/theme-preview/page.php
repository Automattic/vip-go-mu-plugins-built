...
global $gutenberg_theme_preview_routes, $gutenberg_theme_preview_menu_items;
$gutenberg_theme_preview_routes     = array();
$gutenberg_theme_preview_menu_items = array();

/**
 * Register a route for the theme-preview page.
 *
 ...
 */
function gutenberg_register_theme_preview_route( $path, $content_module = null, $route_module = null ) {
	global $gutenberg_theme_preview_routes;
	...
	$gutenberg_theme_preview_routes[] = $route;
}

/**
 * Register a menu item for the theme-preview page.
 *
 ...
 */
function gutenberg_register_theme_preview_menu_item( $id, $label, $to, $parent_id = '', $parent_type = '' ) {
	global $gutenberg_theme_preview_menu_items;
	...
	$gutenberg_theme_preview_menu_items[] = $menu_item;
}

/**
 * Get all registered routes for the theme-preview page.
 *
 * @return array Array of route objects.
 */
function gutenberg_get_theme_preview_routes() {
	global $gutenberg_theme_preview_routes;
	return $gutenberg_theme_preview_routes ?? array();
}

/**
 * Get all registered menu items for the theme-preview page.
 *
 * @return array Array of menu item objects.
 */
function gutenberg_get_theme_preview_menu_items() {
	global $gutenberg_theme_preview_menu_items;
	return $gutenberg_theme_preview_menu_items ?? array();
}

/**
 * Preload REST API data for the theme-preview page.
 * Automatically called during page rendering.
 */
function gutenberg_theme_preview_preload_data() {
 ...
 */
function gutenberg_theme_preview_render_page() {
	...
	/**
	 * Fires when the theme-preview page is initialized so extensions can register routes and menu items.
	 */
	do_action( 'theme-preview_init' );
	...
	gutenberg_theme_preview_preload_data();

	// Get all registered routes and menu items
	$menu_items = gutenberg_get_theme_preview_menu_items();
	$routes = gutenberg_get_theme_preview_routes();
	...
	if ( file_exists( $asset_file ) ) {
		...
		wp_register_script( 'theme-preview-prerequisites', '', $asset['dependencies'], $asset['version'], true );

		// Add inline script to initialize the app
		$init_modules = [];
		wp_add_inline_script(
			'theme-preview-prerequisites',
			sprintf(
		...
		wp_register_style( 'theme-preview-prerequisites', false, $style_dependencies, $asset['version'] );

		// Build dependencies for theme-preview module
		...
		$boot_dependencies = apply_filters(
			'theme-preview_boot_dependencies',
			$boot_dependencies
		);
		...
		wp_enqueue_script( 'theme-preview-prerequisites' );
		wp_enqueue_script_module( 'theme-preview' );
		wp_enqueue_style( 'theme-preview-prerequisites' );
	}
	...
	<body class="theme-preview no-js">
	<?php
	// BEGIN see wp-admin/admin-header.php
	?>
	...
	?>
		...
		<div id="theme-preview-app" style="height: 100vh; box-sizing: border-box;"></div>
	<?php
...
add_action( 'admin_init', 'gutenberg_theme_preview_intercept_render' );
