...
//# sourceURL=`+A.responseUrl),A.blobUrl=sA(B),A.source=void 0}var OA=/\n\/\/# source(Mapping)?URL=([^\n]+)\s*((;|\/\/[^#][^\n]*)\s*)*$/;function nA(A,e,t){let Q=h[A];if(Q)return Q;if(Q={url:A},h[A]){let o=0;for(;h[Q.url+ ++o];);Q.url+=o}return h[Q.url]=Q,Q.fetchPromise=(async()=>{let o;({responseUrl:Q.responseUrl,source:o}=await($A[A]||BA(A,e,t)));try{Q.analysis=q(o,Q.url)}catch(B){console.error(B),Q.analysis=[[],[],!1,!1]}return Q.source=o,Q})(),Q.linkPromise=Q.fetchPromise.then(async()=>{let o=e;Q.deps=(await Promise.all(Q.analysis[0].map(async({n:B,d:E})=>{if(E!==-1||!B)return;let i=D(B,Q.responseUrl||Q.url);return CA&&CA(i)?{blobUrl:i}:(o.integrity&&(o={...o,integrity:void 0}),nA(i,o,Q.responseUrl).fetchPromise)}))).filter(B=>B)}),Q}var rA=A=>import(A);async function P(A,e){await J;let t=nA(A,e,null),Q={};return await gA(t,Q),aA(t,Q),await Promise.resolve(),t}async function y(A){let e=await rA(A.blobUrl);return A.shellUrl&&(await rA(A.shellUrl)).u$_(e),e}async function cA(A,e){let t=await P(A,e);return y(t)}var XA=document.baseURI,IA=XA;Object.defineProperty(self,"wpInteractivityRouterImport",{value:TA,writable:!1,enumerable:!1,configurable:!1});async function TA(A){return await J,cA(D(A,IA),{credentials:"same-origin"})}async function lA(A,e){return tA(e),await J,P(D(A,IA),{credentials:"same-origin"})}var wA=new Set,pA=A=>{wA.add(A)},fA=A=>{let e=A.querySelector("script#wp-importmap[type=importmap]"),t=e?JSON.parse(e.text):{imports:{},scopes:{}};for(let o in m.imports)delete t.imports[o];return[...A.querySelectorAll("script[type=module][src][data-wp-router-options]")].filter(o=>{try{return JSON.parse(o.getAttribute("data-wp-router-options"))?.loadOnClientNavigation===!0}catch{return!1}}).map(o=>o.src).filter(o=>!wA.has(o)).map(o=>lA(o,t))},dA=A=>Promise.all(A.map(e=>y(e)));var{getRegionRootFragment:ZA,initialVdom:VA,toVdom:WA,render:zA,parseServerData:_A,populateServerData:Ae,batch:T,routerRegions:b,h:ee,navigationSignal:Qe,warn:ke}=jA("I acknowledge that using private APIs means my theme or plugin will inevitably break in the next version of WordPress."),R="data-wp-router-region",$="data-wp-interactive",LA=`[${$}][${R}], [${$}] [${$}][${R}]`,p=new Map,N=A=>{let e=new URL(A,window.location.href);return e.pathname+e.search},NA=A=>{let e=A.getAttribute(R);try{let{id:t,attachTo:Q}=JSON.parse(e);return{id:t,attachTo:Q}}catch{return{id:e}}},te=A=>{if(!A)return A;let e=A.props.priorityLevels,t=e.findIndex(o=>o.includes("router-region")),Q=t!==-1?e.slice(t+1):e;return Q.length>0?ee(A.type,{...A.props,priorityLevels:Q}):A.props.element},v=new WeakMap,O=new WeakMap,JA=new Set,oe=async(A,{html:e})=>{try{if(!e){let Q=await window.fetch(A);if(Q.status!==200)return!1;e=await Q.text()}let t=new window.DOMParser().parseFromString(e,"text/html");return await yA(A,t)}catch{return!1}},yA=async(A,e,{vdom:t}={})=>{e.querySelectorAll("noscript").forEach(C=>C.remove());let Q={},o={};e.querySelectorAll(LA).forEach(C=>{let{id:g,attachTo:r}=NA(C);C.parentElement.closest(`[${R}]`)?Q[g]=void 0:Q[g]=t?.has(C)?t.get(C):WA(C),r&&!JA.has(g)&&(o[g]=r)});let B=e.querySelector("title")?.innerText,E=_A(e),[i,s]=await Promise.all([Promise.all(z(e)),Promise.all(fA(e))]);return{regions:Q,regionsToAttach:o,styles:i,scriptModules:s,title:B,initialData:E,url:A}},vA=A=>{_(A.styles);let e={...A.regionsToAttach};T(()=>{Ae(A.initialData),Qe.value+=1,b.forEach(Q=>{Q.value=null});let t=new Set;for(let Q in e){let o=document.querySelector(e[Q]);v.has(o)||v.set(o,[]);let B=v.get(o);B.includes(Q)||(B.push(Q),t.add(o))}for(let Q in A.regions)b.has(Q)&&(b.get(Q).value=te(A.regions[Q]));t.forEach(Q=>{let B=v.get(Q).map(i=>A.regions[i]);if(!O.has(Q)){let i=B.map(({props:s,type:C})=>{let g=typeof C=="function"?s.type:C,r=document.createElement(g);return Q.appendChild(r),r});O.set(Q,ZA(i))}let E=O.get(Q);zA(B,E)})}),A.title&&(document.title=A.title)},KA=A=>(window.location.assign(A),new Promise(()=>{}));window.addEventListener("popstate",async()=>{let A=N(window.location.href),e=p.has(A)&&await p.get(A);e?T(()=>{w.url=window.location.href,vA(e)}):window.location.reload()});document.querySelectorAll(LA).forEach(A=>{let{id:e,attachTo:t}=NA(A);t&&JA.add(e)});window.document.querySelectorAll("script[type=module][src]").forEach(({src:A})=>pA(A));p.set(N(window.location.href),Promise.resolve(yA(N(window.location.href),document,{vdom:VA})));var X="",kA=!1,L={loading:"Loading page, please wait.",loaded:"Page Loaded."},{state:DA}=mA("core/router/private",{state:{navigation:{hasStarted:!1,hasFinished:!1}}},{lock:!0}),{state:w,actions:Be}=mA("core/router",{state:{get navigation(){return DA.navigation}},actions:{*navigate(A,e={}){let{clientNavigationDisabled:t}=uA();t&&(yield KA(A));let Q=N(A),{navigation:o}=DA,{loadingAnimation:B=!0,screenReaderAnnouncement:E=!0,timeout:i=1e4}=e;X=A,Be.prefetch(Q,e);let s=new Promise(r=>setTimeout(r,i)),C=setTimeout(()=>{X===A&&(B&&(o.hasStarted=!0,o.hasFinished=!1),E&&hA("loading"))},400),g=yield Promise.race([p.get(Q),s]);if(clearTimeout(C),X===A)if(g&&!g.initialData?.config?.["core/router"]?.clientNavigationDisabled){yield dA(g.scriptModules),T(()=>{w.url=A,B&&(o.hasStarted=!1,o.hasFinished=!0),vA(g)}),window.history[e.replace?"replaceState":"pushState"]({},"",A),E&&hA("loaded");let{hash:r}=new URL(A,window.location.href);r&&document.querySelector(r)?.scrollIntoView()}else yield KA(A)},*prefetch(A,e={}){let{clientNavigationDisabled:t}=uA();if(t)return;let Q=N(A);(e.force||!p.has(Q))&&p.set(Q,oe(Q,{html:e.html})),yield p.get(Q)}}});w.url=w.url||window.location.href;function hA(A){if(!kA){kA=!0;let t=document.getElementById("wp-script-module-data-@wordpress/interactivity-router")?.textContent;if(t)try{let Q=JSON.parse(t);typeof Q?.i18n?.loading=="string"&&(L.loading=Q.i18n.loading),typeof Q?.i18n?.loaded=="string"&&(L.loaded=Q.i18n.loaded)}catch{}else w.navigation.texts?.loading&&(L.loading=w.navigation.texts.loading),w.navigation.texts?.loaded&&(L.loaded=w.navigation.texts.loaded)}let e=L[A];import("@wordpress/a11y").then(({speak:t})=>t(e),()=>{})}export{Be as actions,w as state};
