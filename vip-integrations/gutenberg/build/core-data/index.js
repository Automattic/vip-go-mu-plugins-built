...
(wp ||= {}).coreData = (() => {
  ...
  // wordpress-external:@wordpress/sync
  var require_sync = __commonJS({
    "wordpress-external:@wordpress/sync"(exports, module) {
  ...
  function getPostChangesFromCRDTDoc(ydoc, editedRecord, _postType) {
    ...
    const changes = Object.fromEntries(
      Object.entries(ymap.toJSON()).filter(([key, newValue]) => {
        ...
        switch (key) {
          case "blocks": {
            if (ydoc.meta?.get(import_sync.CRDT_DOC_META_PERSISTENCE_KEY) && editedRecord.content) {
